...
import { ChevronLeft, ChevronRight, Cake, CalendarHeart, Loader2, AlertCircle } from 'lucide-react';
...
const CalendarPage = () => {
  ...
  const birthdaysThisMonth = contacts
    .filter(c => {
  ...
  const upcomingBirthdays: ContactWithBirthday[] = contacts
    .filter(c => c.birthday)
    .map(c => {
      const bDate = new Date(c.birthday!);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="glass-card-static p-8 text-center animate-in">
          <AlertCircle className="h-12 w-12 text-destructive mx-auto mb-4" />
          <h3 className="font-semibold text-foreground mb-2">Failed to Load Calendar</h3>
          <p className="text-sm text-muted-foreground mb-4">{error}</p>
          <Button onClick={fetchContacts} variant="outline" className="rounded-xl">
            Try Again
          </Button>
        </div>
      ) : (
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={cn(
                      "aspect-square rounded-xl flex flex-col items-center justify-center text-sm relative transition-colors cursor-pointer",
